...
if (!class_exists('EMO_BUPW_Uploader')) {
	class EMO_BUPW_Uploader {
		private $file;
		private $upload_path;
		private $upload_uri;
		...
		private function inspection() {
			if ( in_array( strtolower( pathinfo( $this->filePath, PATHINFO_EXTENSION ) ), $this->allowedExts ) === false ) {
				$this->error = EMO_BUPW_Pass_Error_Msg::error_object( 'file-type', __( "The extension of uploaded file is not allowed, please choose a csv file.", "emo-bulk-update-prices-for-woocommerce" ) );
			}
			if ( $this->file['size'] > $this->allowedsize ) {
				$this->error = EMO_BUPW_Pass_Error_Msg::error_object( 'file-size', __( "File size is more than allowed size.", "emo-bulk-update-prices-for-woocommerce" ) );
			}
		}
		...
		private function upload() {
			...
			if ( move_uploaded_file( $this->file['tmp_name'], $this->filePath ) ) {
				...
				$this->error = EMO_BUPW_Pass_Error_Msg::error_object( 'unable', __( "Unable to upload file", "emo-bulk-update-prices-for-woocommerce" ) );
			}
		}
